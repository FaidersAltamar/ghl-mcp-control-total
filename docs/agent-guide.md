...
## Credentials (see `.env` — never commit)

All secrets live in **one file**: `.env` at the project root. Copy from `.env.example`.

| Variable | Purpose |
|---|---|
| `GHL_PIT_TOKEN` | Public API (contacts, payments, calendars, etc.) |
| `GHL_FIREBASE_REFRESH_TOKEN` | Workflow internal API (auto-renewing) |
| `GHL_LOCATION_ID` | Target sub-account |
| `GHL_COMPANY_ID` | Company ID (optional, some endpoints) |
...
- **Path**: `mcp/workflows/server.js`
- **Transport**: stdio (`node mcp/workflows/server.js`)
...
5. **Never commit credentials** — only `.env` at project root holds secrets (gitignored). Use `.env.example` as template.
...
### 1. SDK-based fallback scripts (created in `tools/api-client/`)
...
Usa `tools/api-client/oauth-helper.js` o los helpers basados en `.env` para intercambiar el auth code o refrescar el token:
...
Y en `tools/api-client/webhooks.log` verás llegar los eventos suscritos.
...
The official API / SDK only lists workflows (`/workflows/`) but does **not** expose the internal triggers, actions, or step tree of a workflow. To get full workflow internals for analysis in OpenCode/Claude Code, use the local Chrome extension in `tools/workflow-extractor/`.
...
cd tools/workflow-extractor
node server.js

# Terminal 2 — load the extension in Chrome:
# 1. Open chrome://extensions, enable Developer mode.
# 2. Click Load unpacked, select tools/workflow-extractor/.
...
See `tools/workflow-extractor/README-OPENCODE.md` for full instructions and troubleshooting.
...
- Created local fallback scripts (`tools/api-client/ghl-client.js`, `tools/api-client/webhook-server.js`, `tools/api-client/oauth-helper.js`) using the official SDK.
